...
function paymentsStatus() {
  ...
  const parsedDonations = donations
    .map((item) => {
      ...
      return {
        ...
        currency: sanitizeText(item?.currency || 'BRL', 8).toUpperCase(),
        at: item?.at,
      };
    })
    .filter((item) => item.amount > 0);

  const brlDonations = parsedDonations.filter((item) => item.currency === 'BRL');
  const totalBRL = brlDonations.reduce((acc, item) => acc + item.amount, 0);
  const count = parsedDonations.length;
  const avg = brlDonations.length ? totalBRL / brlDonations.length : 0;
  const byCurrency = {};
  parsedDonations.forEach((item) => {
    byCurrency[item.currency] = Math.round(((byCurrency[item.currency] || 0) + item.amount) * 100) / 100;
  });
  const latestDonationAt = parsedDonations
    .map((item) => item.at)
    .filter(Boolean)
    .sort()
    .pop() || null;
  ...
  const providers = [
    { id: 'stripe', status: 'planned_adapter', readiness: 55 },
    { id: 'paypal', status: 'planned_adapter', readiness: 52 },
    { id: 'local_support', status: 'active_local', readiness: 78 },
  ];
  ...
  return {
    ...
    payments: {
      totals: {
        transactions: count,
        grossBRL: Math.round(totalBRL * 100) / 100,
      ...
      providers,
      milestones: [
        'Adicionar webhooks de confirmação e reconciliação de transações.',
        'Implementar ledger de eventos financeiros para auditoria detalhada.',
        'Criar relatórios de receita por origem (doação vs serviço premium).',
      ],
...
function marketplaceStatus() {
  ...
  requests.forEach((item) => {
    const role = sanitizeText(item.role, 60) || 'unknown';
  ...
  return {
    ...
    marketplace: {
      totals: {
        demands: requests.length,
        uniqueRoles: Object.keys(byRole).length,
      },
      topRoles,
      latestDemands: requests.slice(-10).reverse(),
      milestones: [
        'Implementar perfis dedicados de freelancer com portfolio estruturado.',
        'Adicionar fluxo de proposta/contraproposta com status por etapa.',
        'Incluir mensageria interna vinculada a demandas/projetos.',
      ],
      generatedAt: new Date().toISOString(),
    },
  };
...
function aiPipelineStatus() {
  const db = readDb();
  const jobs = db.creationJobs || [];
  const byType = { text: 0, image: 0, video: 0, audio: 0 };
  jobs.forEach((job) => {
    const t = sanitizeText(job.type, 20).toLowerCase();
    if (Object.prototype.hasOwnProperty.call(byType, t)) byType[t] += 1;
  });

  const providers = [
    { id: 'renderforest', domain: 'video', status: 'planned_adapter' },
    { id: 'heygen', domain: 'video-avatar', status: 'planned_adapter' },
    { id: 'canva_ai', domain: 'video-design', status: 'planned_adapter' },
    { id: 'internal_text', domain: 'text', status: 'active_local' },
  ];
  ...
  return {
    ...
    pipeline: {
      ...
      providers,
      queue: {
        pending: jobs.filter((j) => sanitizeText(j?.status, 40).toLowerCase().includes('pending')).length,
        completed: jobs.filter((j) => sanitizeText(j?.status, 40).toLowerCase().includes('completed')).length,
      },
      nextMilestones: [
        'Implementar adapters por provedor com timeout/retry padronizado.',
        'Adicionar fila persistente para jobs de mídia pesada.',
        'Salvar metadados de custo/latência por geração para otimização.',
      ],
      generatedAt: new Date().toISOString(),
    },
  };
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/payments/status' && req.method === 'GET') {
    return sendJson(res, 200, paymentsStatus());
  }

  if (pathname === '/api/marketplace/status' && req.method === 'GET') {
    return sendJson(res, 200, marketplaceStatus());
  }

  if (pathname === '/api/ai/pipeline/status' && req.method === 'GET') {
    return sendJson(res, 200, aiPipelineStatus());
  }
